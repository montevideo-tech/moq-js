var moqplayer = (function () {
	...
	async function fetch$1(connection, namespace) {
	    ...
	    try {
	        ...
	        if (!chunk)
	            throw new Error("no catalog chunk");
	        await segment.close();
	...
	function isRoot(catalog) {
	    if (!isCatalogFieldValid(catalog, "packaging"))
	        return false;
	    if (!isCatalogFieldValid(catalog, "namespace"))
	        return false;
	    if (!Array.isArray(catalog.tracks))
	        return false;
	    return catalog.tracks.every((track) => isTrack(track));
	}
	...
	function isVideoSelectionParams(params) {
	...
	function isAudioSelectionParams(params) {
	    if (typeof params.channelConfig !== "string")
	        return false;
	    if (typeof params.samplerate !== "number")
	        return false;
	    return true;
	}
	function isCatalogFieldValid(catalog, field) {
	    ...
	    function isValidPackaging(packaging) {
	...
	function createURL$1(fn, sourcemapArg) {
	...
	function createInlineAudioWorkletFactory(fn, sourcemapArg) {
	    ...
	    return createInlineAudioWorkletFactory$1(fn);
	}

	var WorkerFactory$1 = /*#__PURE__*/createInlineAudioWorkletFactory(/* rollup-plugin-web-worker-loader */function () {
	(function () {
	...
	class Audio {
	    ...
	    async load(config) {
	        ...
	        await WorkerFactory$1(this.context);
	...
	function createInlineWorkerFactory(fn, sourcemapArg) {
	    if (isNodeJS()) {
	        return createInlineWorkerFactory$2(fn);
	    }
	    return createInlineWorkerFactory$1(fn);
	}
	...
	(function () {
		...
		*/
		...
		var mp4box_all_3 = mp4box_all.DataStream;
		mp4box_all.MultiBufferStream;
		mp4box_all.MPEG4DescriptorParser;
		var mp4box_all_6 = mp4box_all.BoxParser;
		mp4box_all.VTTin4Parser;
		mp4box_all.XMLSubtitlein4Parser;
		mp4box_all.Textin4Parser;
		mp4box_all.ISOFile;
		var mp4box_all_11 = mp4box_all.createFile;
		...
		class Parser {
		    ...
		    constructor(init) {
		        ...
		        this.#mp4.onSamples = (_track_id, track, samples) => {
		...
		mp4box_all_6.dOpsBox.prototype.write = function (stream) {
		    ...
		    #transform(frame) {
		        ...
		        if (this.#decoderConfig && this.#decoder.state == "configured") {
		            if (isVideoTrack(track)) {
		                const configMismatch = this.#decoderConfig.codec !== track.codec ||
		        ...
		        if (this.#decoder.state !== "configured") {
		            ...
		            const buffer = new mp4box_all_3(undefined, 0, mp4box_all_3.BIG_ENDIAN);
		            box.write(buffer);
		...
		var StreamType;
		(function (StreamType) {
		    StreamType[StreamType["Object"] = 0] = "Object";
		    StreamType[StreamType["Track"] = 80] = "Track";
		    StreamType[StreamType["Group"] = 81] = "Group";
		})(StreamType || (StreamType = {}));
		var Status;
		(function (Status) {
		    Status[Status["OBJECT_NULL"] = 1] = "OBJECT_NULL";
		    Status[Status["GROUP_NULL"] = 2] = "GROUP_NULL";
		    Status[Status["GROUP_END"] = 3] = "GROUP_END";
		    Status[Status["TRACK_END"] = 4] = "TRACK_END";
		})(Status || (Status = {}));
		class GroupReader {
		    header;
		    stream;
		...
		class Worker {
		    ...
		    #onConfig(msg) {
		        if (msg.audio) {
		            this.#audio = new Renderer$1(msg.audio, this.#timeline.audio);
		        }
		        if (msg.video) {
		            this.#video = new Renderer(msg.video, this.#timeline.video);
		        }
		    }
		    #onInit(msg) {
		    ...
		    async #onSegment(msg) {
		        ...
		        const container = new Parser(await init.promise);
	...
	var Version;
	(function (Version) {
	    Version[Version["DRAFT_00"] = 4278190080] = "DRAFT_00";
	    Version[Version["DRAFT_01"] = 4278190081] = "DRAFT_01";
	    Version[Version["DRAFT_02"] = 4278190082] = "DRAFT_02";
	    Version[Version["DRAFT_03"] = 4278190083] = "DRAFT_03";
	    Version[Version["DRAFT_04"] = 4278190084] = "DRAFT_04";
	    Version[Version["DRAFT_05"] = 4278190085] = "DRAFT_05";
	    Version[Version["KIXEL_00"] = 765184] = "KIXEL_00";
	    Version[Version["KIXEL_01"] = 765185] = "KIXEL_01";
	})(Version || (Version = {}));
	let Stream$1 = class Stream {
	    recv;
	    send;
	    constructor(r, w) {
	        this.recv = new Decoder$1(r);
	        this.send = new Encoder$1(w);
	    }
	};
	let Decoder$1 = class Decoder {
	    ...
	    async client() {
	        ...
	        if (type !== 0x40)
	            throw new Error(`client SETUP type must be 0x40, got ${type}`);
	    ...
	    async server() {
	        const type = await this.r.u53();
	        if (type !== 0x41)
	            throw new Error(`server SETUP type must be 0x41, got ${type}`);
	        const version = await this.r.u53();
	        const params = await this.parameters();
	        return {
	            version,
	    ...
	    role(raw) {
	        ...
	        if (raw.length != 1)
	            throw new Error("multi-byte varint not supported");
	        switch (raw[0]) {
	            case 1:
	                return "publisher";
	            case 2:
	...
	function isPublisher(m) {
	    return (m.kind == Msg.SubscribeOk ||
	...
	(function (Msg) {
	    ...
	    Msg["SubscribeOk"] = "subscribe_ok";
	    Msg["SubscribeError"] = "subscribe_error";
	    Msg["SubscribeDone"] = "subscribe_done";
	    Msg["Unsubscribe"] = "unsubscribe";
	    Msg["Announce"] = "announce";
	    Msg["AnnounceOk"] = "announce_ok";
	    Msg["AnnounceError"] = "announce_error";
	...
	(function (Id) {
	    ...
	    // Object = 0,
	    // Setup = 1,
	    Id[Id["Subscribe"] = 3] = "Subscribe";
	    Id[Id["SubscribeOk"] = 4] = "SubscribeOk";
	    Id[Id["SubscribeError"] = 5] = "SubscribeError";
	    Id[Id["SubscribeDone"] = 11] = "SubscribeDone";
	    Id[Id["Unsubscribe"] = 10] = "Unsubscribe";
	    Id[Id["Announce"] = 6] = "Announce";
	    Id[Id["AnnounceOk"] = 7] = "AnnounceOk";
	    Id[Id["AnnounceError"] = 8] = "AnnounceError";
	    Id[Id["Unannounce"] = 9] = "Unannounce";
	    Id[Id["GoAway"] = 16] = "GoAway";
	})(Id || (Id = {}));
	var GroupOrder;
	(function (GroupOrder) {
	    GroupOrder[GroupOrder["Publisher"] = 0] = "Publisher";
	...
	class Objects {
	    ...
	    async send(h) {
	        ...
	        if (h.type == StreamType.Object) {
	            ...
	            res = new ObjectWriter(h, w);
	        }
	        else if (h.type === StreamType.Group) {
	            await w.u53(h.group);
	            await w.u8(h.publisher_priority);
	            res = new GroupWriter(h, w);
	        }
	        else if (h.type === StreamType.Track) {
	            await w.u8(h.publisher_priority);
	            res = new TrackWriter(h, w);
	        }
	        else {
	...
	class TrackWriter {
	    ...
	    async write(c) {
	        ...
	        if (c.payload instanceof Uint8Array) {
	            ...
	            await this.stream.u53(c.payload);
	        }
	    }
	    async close() {
	        await this.stream.close();
	...
	class Publisher {
	    ...
	    async recv(msg) {
	        if (msg.kind == Msg.Subscribe) {
	            await this.recvSubscribe(msg);
	        }
	        else if (msg.kind == Msg.Unsubscribe) {
	            this.recvUnsubscribe(msg);
	        }
	        else if (msg.kind == Msg.AnnounceOk) {
	            this.recvAnnounceOk(msg);
	        }
	        else if (msg.kind == Msg.AnnounceError) {
	...
	class AnnounceSend {
	    ...
	    #state = new Watch("init");
	    constructor(control, namespace) {
	        this.#control = control;
	...
	class SubscribeRecv {
	    ...
	    #state = "init";
	    constructor(control, objects, msg) {
	...
	class Subscriber {
	    ...
	    async recv(msg) {
	        ...
	        else if (msg.kind == Msg.Unannounce) {
	            this.recvUnannounce(msg);
	        }
	        else if (msg.kind == Msg.SubscribeOk) {
	            this.recvSubscribeOk(msg);
	        }
	        else if (msg.kind == Msg.SubscribeError) {
	            await this.recvSubscribeError(msg);
	        }
	        else if (msg.kind == Msg.SubscribeDone) {
	    ...
	    async unsubscribe(track) {
	        if (this.#trackToIDMap.has(track)) {
	            ...
	            try {
	                await this.#control.send({ kind: Msg.Unsubscribe, id: trackID });
	...
	class AnnounceRecv {
	    ...
	    async ok() {
	        ...
	        this.#state = "ack";
	        // Send the control message.
	        return this.#control.send({ kind: Msg.AnnounceOk, namespace: this.namespace });
	    }
	    async close(code = 0n, reason = "") {
	        if (this.#state === "closed")
	            return;
	        this.#state = "closed";
	        return this.#control.send({ kind: Msg.AnnounceError, namespace: this.namespace, code, reason });
	    }
	}
	class SubscribeSend {
	    #control;
	    ...
	    #data = new Queue();
	    constructor(control, id, namespace, track) {
	        this.#control = control; // so we can send messages
	        this.#id = id;
	        this.namespace = namespace;
	        this.track = track;
	...
	class Connection {
	    ...
	    #running;
	    constructor(quic, control, objects) {
	        this.#quic = quic;
	        this.#control = control;
	        this.#objects = objects;
	...
	class Client {
	    ...
	    async connect() {
	        ...
	        const setup = new Stream$1(reader, writer);
	        // Send the setup message.
	        await setup.send.client({ versions: [Version.DRAFT_05], role: this.config.role });
	        // Receive the setup message.
	        // TODO verify the SETUP response.
	        const server = await setup.recv.server();
	        if (server.version != Version.DRAFT_05) {
	            throw new Error(`unsupported server version: ${server.version}`);
	        }
	        const control = new Stream(reader, writer);
	        const objects = new Objects(quic);
	        return new Connection(quic, control, objects);
	    }
	    async #fetchFingerprint(url) {
	        if (!url)
	            return;
	...
	class Player {
	    ...
	    #abort;
	    #trackTasks = new Map();
	    constructor(connection, catalog, backend, tracknum) {
	    ...
	    async #run() {
	        ...
	        this.#catalog.tracks.forEach((track, index) => {
	            if (index == this.#tracknum || isAudioTrack(track)) {
	                if (!track.namespace)
	                    throw new Error("track has no namespace");
	                if (track.initTrack)
	                    inits.add([track.namespace, track.initTrack]);
	                tracks.push(track);
	            }
	        });
	    ...
	    async #runInit(namespace, name) {
	        ...
	        try {
	            ...
	            if (!chunk)
	                throw new Error("no init chunk");
	            if (!(chunk.payload instanceof Uint8Array))
	                throw new Error("invalid init chunk");
	    ...
	    async #trackTask(track) {
	        if (!track.namespace)
	            throw new Error("track has no namespace");
	        if (this.#paused)
	            return;
	        const kind = isVideoTrack(track) ? "video" : isAudioTrack(track) ? "audio" : "unknown";
	    ...
	    getVideoTracks() {
	        return this.#catalog.tracks.filter(isVideoTrack).map((track) => track.name);
	    }
	    getAudioTracks() {
	        return this.#catalog.tracks.filter(isAudioTrack).map((track) => track.name);
	    }
	    ...
	    async unsubscribeFromTrack(trackname) {
	    ...
	    subscribeFromTrackName(trackname) {
	        console.log(`Subscribing to track: ${trackname}`);
...
//# sourceMappingURL=moq-player.umd.js.map
